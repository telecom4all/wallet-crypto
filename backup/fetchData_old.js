...
// useFetchData est maintenant un véritable custom Hook.
const useFetchData = (
  setLoggedInValue,
  setEmailValue,
  setFreeValue,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const loggedIn = storedLoggedIn ? cleanStorageData(storedLoggedIn) : null;
      ...
      setMaticWalletValue(storedMaticWallet ? cleanStorageData(storedMaticWallet) : null);
      ...
      if (loggedIn) {
        ...
        if (cryptoResponse.success === true) {
          ...
          showNotification(cryptoResponse.message || "Erreur de réception de la liste des cryptos", "error");
        }
        ...
        if (transactionResponse.success === true) {
          ...
          showNotification(transactionResponse.message || "Erreur de réception de la liste des transactions", "error");
        }
      }
    } catch (error) {
